...
6. Reviewer confidence analysis
   ...
   - Two-panel manuscript figure uses field-cluster bootstrap confidence intervals
...
FILEPATH_FOR_LOCATIONS = Path(
    r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\AI_Matched_PointPairs_Observations\stads_scouting_validation_analysis_all_point_data_with_evaluations.csv"
)
...
CLASSIFIER_FILEPATHS = [
    Path(r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\IndependentClassifications\BlindAnomalyClassification_AY.xlsx"),
    Path(r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\IndependentClassifications\BlindAnomalyClassification_LL.xlsx"),
    Path(r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\IndependentClassifications\BlindAnomalyClassification_DL.xlsx"),
    Path(r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\IndependentClassifications\BlindAnomalyClassification_PL.xlsx"),
    Path(r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\IndependentClassifications\BlindAnomalyClassification_KR.xlsx"),
]
...
RTK_GPKG_PATH = (r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Data\CorrectedForRTKScoutingMismatchIssues\RTKGuidedScouting_corrected_analysis_included.geojson")
RTK_GPKG_FILENAME = "RTKGuidedScouting_corrected_analysis_included.geojson"
RTK_GPKG_LAYER = None
...
OUTPUT_DIR = Path(
    r"G:\My Drive\Programming\AnomalyDetectionScoutingAnalysis\Figures\ReviewerAssessment_Analysis"
)

# Field-cluster bootstrap confidence intervals generated by
# stads_field_cluster_bootstrap_ci_blind_rtk_farms_fixed.py.
#
# The two-panel reviewer-confidence figure below uses this workbook for its
# confidence intervals instead of the unadjusted Wilson intervals generated by
# summarize_binary_rate(). The point estimates and denominators are checked
# against the main analysis before plotting.
BOOTSTRAP_CI_WORKBOOK = (
    OUTPUT_DIR
    / "FieldClusterBootstrap"
    / "field_cluster_bootstrap_confidence_intervals.xlsx"
)

USE_BOOTSTRAP_CI_FOR_CONFIDENCE_FIGURE = True

BOOTSTRAP_CI_LOWER_PERCENT_COL = "field_cluster_bootstrap_ci_lower_percent"
BOOTSTRAP_CI_UPPER_PERCENT_COL = "field_cluster_bootstrap_ci_upper_percent"

BOOTSTRAP_ANOMALY_CONFIDENCE_ANALYSIS = (
    "Reviewer confidence vs anomaly-consensus agreement"
)
BOOTSTRAP_DIRECTION_CONFIDENCE_ANALYSIS = (
    "Reviewer direction confidence vs direction agreement"
)
...
OPTIONAL_CONTEXT_COLS = [
    ...
    "X",
    "Y",
    # Corrected-source audit and authoritative planned-point metadata
    "raw_sample_pai",
    "raw_a_or_b",
    "my_in_or_out",
    "stads_direction",
    "fieldid",
    "planned_deviance_sign",
    "planned_scene_date",
    "planned_lon",
    "planned_lat",
    "planned_source_file",
    "planned_source_fid",
    "planned_metadata_status",
    "replacement_pair",
    "survey_coordinates_valid",
    "geometry_coordinate_source",
...
# Publication figure colors. Text and error bars use dark gray; axis lines and
# tick marks use a lighter gray to match the manuscript figure style.
PLOT_TEXT_COLOR = "#4D4D4D"
PLOT_ERRORBAR_COLOR = "#4D4D4D"
PLOT_AXIS_COLOR = "#BFBFBF"
PLOT_SEPARATOR_COLOR = "#D9D9D9"
...
def derive_joined_rtk_anomaly_fields(rtk: pd.DataFrame) -> pd.DataFrame:
    ...
    # The corrected RTK source contains authoritative inside/outside and STADS
    # direction values joined from the planned point-pair layers. Preserve them
    # before deriving fallback values from spatially joined anomaly attributes.
    explicit_in_or_out = (
        rtk[IN_OR_OUT_COL_NAME].copy()
        if IN_OR_OUT_COL_NAME in rtk.columns
        else pd.Series(pd.NA, index=rtk.index, dtype="object")
    )
    explicit_stads_direction = (
        rtk[ACTUAL_DIRECTION_COL_NAME].copy()
        if ACTUAL_DIRECTION_COL_NAME in rtk.columns
        else pd.Series(pd.NA, index=rtk.index, dtype="object")
    )
    ...
    explicit_in_or_out_normalized = explicit_in_or_out.astype("string").str.strip().str.casefold()
    explicit_in_or_out_valid = explicit_in_or_out_normalized.isin(["in", "out"])
    rtk.loc[explicit_in_or_out_valid, IN_OR_OUT_COL_NAME] = explicit_in_or_out_normalized[
        explicit_in_or_out_valid
    ]
    ...
    explicit_direction_normalized = explicit_stads_direction.apply(normalize_direction_value)
    explicit_direction_valid = explicit_direction_normalized.isin(["Positive", "Negative"])
    rtk.loc[explicit_direction_valid, ACTUAL_DIRECTION_COL_NAME] = explicit_direction_normalized[
        explicit_direction_valid
    ]
    rtk.loc[explicit_direction_valid, "rtk_stads_direction_source_col"] = (
        "corrected_planned_point_source"
    )
...
def load_rtk_guided_gpkg(
    ...
    if gpkg_path.suffix.casefold() in {".geojson", ".json"}:
        # The corrected handoff uses GeoJSON. Read it with the standard library
        # so the RTK portion does not require geopandas/GDAL merely for I/O.
        geojson = json.loads(gpkg_path.read_text(encoding="utf-8-sig"))
        records = []
...
def style_rate_axis(ax: plt.Axes, ylabel: Optional[str] = None) -> None:
    """Apply shared publication styling to rate plots."""
    if ylabel is not None:
        ax.set_ylabel(ylabel, fontsize=11, color=PLOT_TEXT_COLOR)

    ax.set_ylim(0, 100)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)

    # Keep label text dark gray, but use the lighter gray for axis lines and
    # tick marks so the axes do not visually compete with the data.
    for spine_name in ["left", "bottom"]:
        ax.spines[spine_name].set_color(PLOT_AXIS_COLOR)
        ax.spines[spine_name].set_linewidth(0.8)

    ax.tick_params(
        axis="both",
        color=PLOT_AXIS_COLOR,
...
def add_rate_bar_labels(
    ax: plt.Axes,
    x: np.ndarray,
    y: pd.Series,
    ci_lower_percent: pd.Series,
    ci_upper_percent: pd.Series,
) -> None:
    """Add centered percent and confidence-interval labels inside bars."""
    for xi, yi, ci_lo, ci_hi in zip(
        x,
        y,
        ci_lower_percent,
        ci_upper_percent,
...
def prepare_confidence_plot_df(
    summary: pd.DataFrame,
    x_col: str,
    rate_col: str,
) -> pd.DataFrame:
    """Prepare High/Medium/Low confidence summaries in the desired order."""
    if summary.empty or rate_col not in summary.columns or x_col not in summary.columns:
        return pd.DataFrame()

    confidence_order = ["High", "Medium", "Low"]
    plot_df = summary.dropna(subset=[rate_col]).copy()
    plot_df = plot_df[plot_df[x_col].isin(confidence_order)].copy()
    if plot_df.empty:
        return plot_df
    ...
    return plot_df.sort_values(x_col).reset_index(drop=True)
...
def plot_rate_with_ci(
    ...
    # Put confidence categories in logical rather than alphabetical order.
    confidence_order = ["High", "Medium", "Low"]
    if "confidence" in x_col.lower():
        plot_df = plot_df[plot_df[x_col].isin(confidence_order)].copy()
        plot_df[x_col] = pd.Categorical(
            plot_df[x_col],
            categories=confidence_order,
    ...
    if hue_col and hue_col in plot_df.columns:
        ...
        for hue_i, hue in enumerate(unique_hues):
            ...
            x = np.array([x_lookup[str(v)] for v in sub[x_col]]) + (
                hue_i - (len(unique_hues) - 1) / 2
            ) * width
            y = sub[rate_col].astype(float) * 100
            ci_lo = sub["ci_lower"].astype(float) * 100
            ci_hi = sub["ci_upper"].astype(float) * 100
            ...
            ax.errorbar(
                ...
                capsize=3,
                linewidth=1,
                ecolor=PLOT_ERRORBAR_COLOR,
                zorder=3,
            )
            add_rate_bar_labels(ax, x, y, ci_lo, ci_hi)
        ...
        ci_lo = plot_df["ci_lower"].astype(float) * 100
        ci_hi = plot_df["ci_upper"].astype(float) * 100
        yerr = np.vstack([y - ci_lo, ci_hi - y])
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(plot_df[x_col].astype(str), rotation=0, ha="center")

    ax.set_title(title, fontsize=12, color=PLOT_TEXT_COLOR)
    style_rate_axis(ax, ylabel=ylabel)

    legend = ax.get_legend()
    if legend is not None:
        plt.setp(legend.get_texts(), color=PLOT_TEXT_COLOR)
...
def load_bootstrap_ci_table() -> pd.DataFrame:
    """Load the field-cluster bootstrap CI workbook used by manuscript plots.

    The workbook is required when USE_BOOTSTRAP_CI_FOR_CONFIDENCE_FIGURE=True.
    Failure is explicit rather than silently falling back to Wilson intervals.
    """
    if not USE_BOOTSTRAP_CI_FOR_CONFIDENCE_FIGURE:
        return pd.DataFrame()

    if not BOOTSTRAP_CI_WORKBOOK.exists():
        raise FileNotFoundError(
            "Field-cluster bootstrap CI workbook was not found:\n"
            f"{BOOTSTRAP_CI_WORKBOOK}\n\n"
            "Run the bootstrap-CI analysis first, or update "
            "BOOTSTRAP_CI_WORKBOOK in the CONFIG section."
        )

    xls = pd.ExcelFile(BOOTSTRAP_CI_WORKBOOK)

    preferred_sheet = "all_bootstrap_CIs"
    if preferred_sheet in xls.sheet_names:
        ...
        # Fall back to the first sheet that contains the required columns.
        table = pd.DataFrame()
        for sheet_name in xls.sheet_names:
            ...
            required = {
                ...
                "n",
                "estimate_percent",
                BOOTSTRAP_CI_LOWER_PERCENT_COL,
                BOOTSTRAP_CI_UPPER_PERCENT_COL,
            }
            if required.issubset(candidate.columns):
                table = candidate
                print(
                    "Bootstrap CI table loaded from fallback sheet: "
                    f"{sheet_name}"
                )
                break
        ...
        if table.empty:
            raise ValueError(
                "No sheet in the bootstrap workbook contains the required "
                "field-cluster CI columns."
            )
    ...
    missing = sorted(required_cols - set(table.columns))
    if missing:
        raise ValueError(
            "Bootstrap CI table is missing required columns: "
            f"{missing}"
        )
...
def apply_bootstrap_ci_to_confidence_summary(
    summary: pd.DataFrame,
    x_col: str,
    rate_col: str,
    bootstrap_table: pd.DataFrame,
    bootstrap_analysis_name: str,
) -> pd.DataFrame:
    """Replace Wilson CI columns with field-cluster bootstrap CIs.

    Point estimates, successes, and denominators remain those produced by the
    main reviewer analysis. The function verifies that they agree with the
    bootstrap workbook before replacing only the CI columns.
    """
    plot_df = prepare_confidence_plot_df(
        summary,
        x_col=x_col,
        rate_col=rate_col,
    )
    ...
    boot = bootstrap_table[
        bootstrap_table["analysis"].astype(str).str.strip()
        == bootstrap_analysis_name
    ].copy()

    boot = boot[
        boot["subgroup_value"].astype(str).isin(
            ["High", "Medium", "Low"]
        )
    ].copy()
    ...
    if boot.empty:
        raise ValueError(
            "No matching bootstrap rows were found for analysis: "
            f"{bootstrap_analysis_name!r}"
        )

    # One bootstrap row is expected for each confidence category.
    duplicate_groups = (
        boot.groupby("subgroup_value", dropna=False)
        .size()
    )
    ...
    if not duplicate_groups.empty:
        raise ValueError(
            "Duplicate bootstrap rows were found for confidence categories: "
            f"{duplicate_groups.to_dict()}"
        )

    boot_lookup = boot.set_index(
        boot["subgroup_value"].astype(str)
    )
    ...
    for _, row in plot_df.iterrows():
        confidence = str(row[x_col])

        if confidence not in boot_lookup.index:
            raise ValueError(
                f"No bootstrap CI row was found for {confidence!r} under "
                f"{bootstrap_analysis_name!r}."
            )

        boot_row = boot_lookup.loc[confidence]

        # Verify denominator and successes whenever the main summary includes
        # those fields. This guards against accidentally plotting bootstrap
        # intervals from an out-of-date workbook.
        if "n" in plot_df.columns:
            main_n = int(row["n"])
            boot_n = int(boot_row["n"])
            if main_n != boot_n:
                raise ValueError(
                    f"Bootstrap denominator mismatch for {confidence} "
                    f"({bootstrap_analysis_name}): "
                    f"main analysis n={main_n}, bootstrap n={boot_n}."
                )

        if "successes" in plot_df.columns:
            main_k = int(row["successes"])
            boot_k = int(boot_row["successes"])
            if main_k != boot_k:
                raise ValueError(
                    f"Bootstrap numerator mismatch for {confidence} "
                    f"({bootstrap_analysis_name}): "
                    f"main analysis successes={main_k}, "
                    f"bootstrap successes={boot_k}."
                )

        main_percent = float(row[rate_col]) * 100.0
        boot_percent = float(boot_row["estimate_percent"])
        if not np.isclose(
            main_percent,
            boot_percent,
            atol=0.051,
            ...
            raise ValueError(
                f"Bootstrap point-estimate mismatch for {confidence} "
                f"({bootstrap_analysis_name}): "
                f"main={main_percent:.3f}%, bootstrap={boot_percent:.3f}%."
            )
    ...
    plot_df["ci_lower"] = ci_lower_values
    plot_df["ci_upper"] = ci_upper_values
    plot_df["ci_method"] = "95% field-cluster bootstrap percentile CI"

    return plot_df


def plot_confidence_two_panel(
    confidence_outputs: dict[str, pd.DataFrame],
    bootstrap_ci_table: Optional[pd.DataFrame] = None,
    filename: str = "reviewer_confidence_agreement_two_panel_bootstrapCI",
) -> None:
    """Create the two-panel reviewer-confidence figure with cluster CIs.

    Panel A:
        anomaly-classification agreement with point-pair consensus.

    Panel B:
        reviewer direction agreement with STADS-derived direction.

    The bars use the point estimates from the main reviewer analysis. When
    USE_BOOTSTRAP_CI_FOR_CONFIDENCE_FIGURE=True, the error bars and CI labels
    are replaced by 95% field-cluster bootstrap percentile confidence intervals
    loaded from BOOTSTRAP_CI_WORKBOOK. No silent Wilson fallback is allowed.
    """
    anomaly_summary = confidence_outputs.get(
        "anomaly_confidence_vs_consensus",
        pd.DataFrame(),
    )
    direction_summary = confidence_outputs.get(
        "direction_confidence_vs_match",
        pd.DataFrame(),
    )

    if USE_BOOTSTRAP_CI_FOR_CONFIDENCE_FIGURE:
        if bootstrap_ci_table is None or bootstrap_ci_table.empty:
            bootstrap_ci_table = load_bootstrap_ci_table()

        anomaly_df = apply_bootstrap_ci_to_confidence_summary(
            anomaly_summary,
            x_col="anomaly_confidence_normalized",
            rate_col="consensus_agreement_rate",
            bootstrap_table=bootstrap_ci_table,
            bootstrap_analysis_name=BOOTSTRAP_ANOMALY_CONFIDENCE_ANALYSIS,
        )

        direction_df = apply_bootstrap_ci_to_confidence_summary(
            direction_summary,
            x_col="direction_confidence_normalized",
            rate_col="direction_agreement_rate",
            bootstrap_table=bootstrap_ci_table,
            bootstrap_analysis_name=BOOTSTRAP_DIRECTION_CONFIDENCE_ANALYSIS,
        )
    else:
        anomaly_df = prepare_confidence_plot_df(
            anomaly_summary,
            x_col="anomaly_confidence_normalized",
            rate_col="consensus_agreement_rate",
        )
        direction_df = prepare_confidence_plot_df(
            direction_summary,
            x_col="direction_confidence_normalized",
            rate_col="direction_agreement_rate",
        )

    if anomaly_df.empty or direction_df.empty:
        print(
            "WARNING: Two-panel confidence figure was not created because one "
            "or both confidence-summary tables were empty."
        )
        return
    ...
    panel_specs = [
        ...
        (
            axes[1],
            direction_df,
            "direction_confidence_normalized",
            "direction_agreement_rate",
            "B.",
            "Agreement with STADS direction (%)",
        ),
    ]

    for ax, plot_df, x_col, rate_col, panel_label, ylabel in panel_specs:
        x = np.arange(len(plot_df))
        y = plot_df[rate_col].astype(float) * 100
        ci_lo = plot_df["ci_lower"].astype(float) * 100
        ci_hi = plot_df["ci_upper"].astype(float) * 100

        # Numerical noise or rounding in a bootstrap percentile bound can
        # occasionally place a bound essentially on the point estimate.
        # Clip to nonnegative error-bar lengths for matplotlib.
        yerr = np.vstack(
            [
                np.maximum(0.0, y.to_numpy() - ci_lo.to_numpy()),
                np.maximum(0.0, ci_hi.to_numpy() - y.to_numpy()),
            ]
        )
        ...
        # No plot titles: identify panels only with A. and B.
        ax.text(
            -0.02,
            1.03,
            panel_label,
    ...
    # Light-gray separator between panels.
    fig.add_artist(
        plt.Line2D(
            [0.5, 0.5],
            [0.12, 0.92],
    ...
    # sharey=True suppresses second-panel y tick labels in some matplotlib
    # versions; retain them because the two panels use different y-axis labels.
    axes[1].tick_params(
        axis="y",
        color=PLOT_AXIS_COLOR,
        labelcolor=PLOT_TEXT_COLOR,
...
def run_plots(df: pd.DataFrame, consensus_outputs: dict[str, pd.DataFrame], direction_outputs: dict[str, pd.DataFrame], confidence_outputs: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    if (
        "anomaly_confidence_vs_consensus" in confidence_outputs
        and "direction_confidence_vs_match" in confidence_outputs
    ):
        bootstrap_ci_table = (
            load_bootstrap_ci_table()
            if USE_BOOTSTRAP_CI_FOR_CONFIDENCE_FIGURE
            else pd.DataFrame()
        )

        plot_confidence_two_panel(
            confidence_outputs,
            bootstrap_ci_table=bootstrap_ci_table,
            filename="reviewer_confidence_agreement_two_panel_bootstrapCI",
        )
